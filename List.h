//
// Created by Sergey on 10/3/2021.
//
...
List::~List() {Clear();} // Определяем диструктор - запускается фунция очистки списка и сообщение о вызове диструктора
...
void List::Insert(int data, int index) {
    if (!head || (index == size)) // Если список пуст или нужно добавить следующий за последним элемент - вызываем фунцию добавитть в конец - чтобы не писать лишний код
        PushBack(data);
...
void List::Clear()
{
    while (head) // Выполнять цикл, пока адреса конца и аначала списка не бедут равны
    {
...
int List::GetSize() const { // Просто возвращаем значение поля размера списка
    return size;
}
int List::Front() {
    return head->data;
